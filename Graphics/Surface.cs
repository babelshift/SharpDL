...
namespace SharpDL.Graphics
{
    public class Surface : ISurface
    {
        ...
        internal Surface(IFont font, string text, Color color, int wrapLength, FontSurfaceType fontSurfaceType = FontSurfaceType.Blended)
        {
            if (font == null)
            {
            ...
            if (wrapLength > 0)
            {
                switch (fontSurfaceType)
                {
                    case FontSurfaceType.Blended:
                        unsafeHandle = SDL_ttf.TTF_RenderText_Blended_Wrapped(font.Handle, text, rawColor, (uint)wrapLength);
                        break;
                    case FontSurfaceType.Shaded:
                        unsafeHandle = SDL_ttf.TTF_RenderText_Shaded_Wrapped(font.Handle, text, rawColor, rawColor, (uint)wrapLength);
                        break;
                    case FontSurfaceType.Solid:
                        unsafeHandle = SDL_ttf.TTF_RenderText_Solid_Wrapped(font.Handle, text, rawColor, (uint)wrapLength);
                        break;
                }
            }
            else
            {
                switch (fontSurfaceType)
                {
                    case FontSurfaceType.Blended:
                        ...
                        break;
                    case FontSurfaceType.Shaded:
                        unsafeHandle = SDL_ttf.TTF_RenderText_Shaded(font.Handle, text, rawColor, rawColor);
                        break;
                    case FontSurfaceType.Solid:
                        unsafeHandle = SDL_ttf.TTF_RenderText_Solid(font.Handle, text, rawColor);
                        break;
                }
            }
